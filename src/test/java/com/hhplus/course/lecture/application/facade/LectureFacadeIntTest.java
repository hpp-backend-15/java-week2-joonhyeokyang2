...
public class LectureFacadeIntTest {
    ...
    void 수강신청자id로_수강신청이_가능하다() throws Exception {
        ...
        //when
        lectureFacade.enroll("lecture1", "joonhyeok", now);

        //then
        Lecture foundLecture = lectureFacade.findById(LectureId.of("lecture1"));
        assertThat(foundLecture).isNotNull();
        assertThat(foundLecture.hasUserOf(UserId.of("joonhyeok"), now)).isTrue();
    }
}
